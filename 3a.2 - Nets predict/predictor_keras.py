...
def get_model_file_aux(model_file_aux_name):
    check_if_file_exists(model_file_aux_name)
...
def get_predictions(input_data_df, model_file, model_file_aux):
    ## Get category id, classifier and CountVectorizer from model file
    category_id = model_file_aux['Category ID']
    #clf         = model_file['Model'] 
    clf         = model_file
    #count_vect  = model_file['Count vectorizer']
    tok = model_file_aux['Tokenizer']
    max_len = model_file_aux['Max length']
    ...
    input_data_df['Prediction']       = input_data_df[col_name].apply(lambda x: predict(x, tok, clf, max_len, category_id))
    input_data_df['Prediction_proba'] = input_data_df[col_name].apply(lambda x: predict_proba(x, tok, clf, max_len))
...
def apply_predictions(clf, tok, max_len, category_id, col_name, input_data_df):
    ## Add 2 columns with predictions and prediction probabilities
    start = time.time()
...
def get_predictions_parallel(input_data_df, model_file, model_file_aux, num_partitions = 2, num_cores = 1):
    ## Get category id, classifier and CountVectorizer from model file
    category_id = model_file_aux['Category ID']
    #clf         = model_file['Model'] 
    clf         = model_file
    ...
    apply_predictions_partial = partial(apply_predictions, clf, tok, max_len, category_id, col_name)
    pool = Pool(num_cores)
...
def insert_df_into_db(insertion_chunk_size, input_data_df, options_):
    ...
    conn_RedShift = psycopg2.connect(host = 'rops.dwh.prod.slicetest.com',
                                     database = 'sliceds',
                                     port = 5439,
                                     user = 'altay', 
                                     password = 'P@s$w0rd!')
...
def main_1():
    ...
    model_file_aux = get_model_file_aux(model_file_aux_name)
    print(model_file_aux)
    ...
    predictions_df = get_predictions(test_data_df, model_file, model_file_aux)
    #predictions_df = get_predictions_parallel(test_data_df, model_file, model_file_aux, num_partitions = 4, num_cores = None)
...
def main_4():
    ...
    # temp1
    #test_data_df = test_data_df.loc[0:1000,:]
    ...
    for r_ in test_data_ranges:
        ...
        ## Get data with prediction results
        predictions_df = get_predictions(test_data_df_part, model_file, model_file_aux)
        #predictions_df = get_predictions_parallel(test_data_df, model_file, model_file_aux, num_partitions = 4, num_cores = None)
