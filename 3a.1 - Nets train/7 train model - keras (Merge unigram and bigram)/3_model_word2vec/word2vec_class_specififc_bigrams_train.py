...
def NGramGenerator_wordwise_interval(phrase, min_ngram, max_ngram):
	...
	#s_split = "".join((char if char in printable_ else "") for char in phrase.lower()).split()
...
if __name__ == "__main__":
	...
	for index, row in sentences_df.iterrows():
		...
		tokens_ls = NGramGenerator_wordwise_interval(sentence,2,2)
		for bigram in tokens_ls:
			bigram_ = "_".join(bigram.split())
			sentences_ls.append([bigram_, row['category_path']])
	print('First sentence tokens:',sentences_ls[:10])
	...
	#5 Save model
	model_name = 'word2vect_class_specififc_bigrams__vec'+str(num_features)+'_win'+str(context)+'__dict_sample_5000'
