...
if __name__ == "__main__":
	...
	for index, row in sentences_df.iterrows():
		...
		for i in range(len(tokens_ls)-1):
			for j in range(i+1,len(tokens_ls)):
				sentences_ls.append([tokens_ls[i],tokens_ls[j]])
		#for token in tokens_ls:
		#	sentences_ls.append([token,row['category_path']])
	print('First sentence tokens:',sentences_ls[:30])
	sentences_df = None
	...
	#5 Save model
	model_name = 'word2vect_unigrams_interrelations__vec'+str(num_features)+'_win'+str(context)+'__dict_sample_5000'
	model.save(model_name)
	print('Model saved as %s \n' % model_name)

	#6 Test Word2Vec model
	test_words = ['screwdriver','pliers','smartphone','baby']
	print('='*100)
